...
import { isProblemItem, rulesConfig } from '../../utils/rules.js';

export default function InspectionDetailModal({ inspection, onClose, onReportIssue }) {
  useDictionaries();
  const ruleConfig = rulesConfig();
    ...
    >
      ...
      <div className="check-grid">
        {(inspection.items || []).map((item) => (
          <div
            className={`check-item${isProblemItem(item, ruleConfig) ? ' is-low' : ''}`}
            key={item.name}
          >
            ...
            <div className="score-line">
              ...
              <span className="muted">
                {!isProblemItem(item, ruleConfig) ? '达标' : '不达标'}
              </span>
            </div>
