...
│   │   ├── services             # 业务规则层：rules（评分/编号/期限/超期单一来源）+ 台账、巡查、问题整改、统计
│   │   ├── seed.py              # 演示数据生成
...
四类核心规则在 **`backend/app/services/rules.py`** 单点实现，录入、查询、统计等所有入口统一从这里取值，不在其它模块重写：

- **巡查评分**：8 个检查项各 0-10 分，得分 = 总得分 / 满分 × 100；≥90 优秀、≥80 良好、≥70 合格，其余不合格。任一检查项低于 6 分或等级为不合格时，巡查结论自动置为「发现问题」。
- **问题编号**：`WT-` + 上报日期 + 当日三位流水号。
- **整改期限**：按严重程度推算默认期限（紧急 1 天，其余 3 天），上报表单与演示数据都取自同一推算函数。
- **超期判定**：整改期限早于当前时间且状态仍处于未闭环（待整改/整改中/待验收）时判定「已超期」；列表筛选、详情标签与看板统计共用同一判定。

规则参数（等级线、单项不合格线、期限天数、未闭环状态）通过 `GET /meta/dictionaries` 下发，前端录入表单的实时预览、超期标签、默认期限均取自该字典，字典未加载时使用的兜底默认值与后端保持一致。因此同一批数据在录入、列表、详情、统计任何入口算出的得分、期限与超期结论都相同；`backend/tests/test_rules.py` 固化了这一不变量。
...
```bash
cd backend && pytest -q          # 接口测试 + 规则层测试（覆盖台账 CRUD、删除保护、评分、流程流转、统计、规则口径一致性）
