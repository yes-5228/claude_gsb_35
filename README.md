...
巡查评分、问题期限推算、超期判定、问题编号生成四组易变规则只在 **`backend/app/services/rules.py`** 定义一份（单一来源）；录入（新增/更新）、列表筛选、看板统计以及未来的导出都必须调用它，不再各自重算。前端不硬编码任何阈值/天数/状态集合，而是从 `/meta/dictionaries` 读取该模块下发的口径后做同公式的实时预览（`frontend/src/utils/rules.js`）。

- **巡查评分**：8 个检查项各 0-10 分，得分 = 总得分 / 满分 × 100；≥90 优秀、≥80 良好、≥70 合格，其余不合格。任一检查项低于 6 分或等级为不合格时，巡查结论自动置为「发现问题」。阈值与分级边界均取自 `rules.py`（`INSPECTION_ITEM_MAX_SCORE`、`INSPECTION_ITEM_PROBLEM_THRESHOLD`、`GRADE_THRESHOLDS`）。
- **问题编号**：`WT-` + 上报日期 + 当日三位流水号，由 `rules.next_issue_code()` 生成，跳号时自动顺延。
- **问题期限**：未显式填期限时按严重程度自上报时间推算（紧急 1 天、严重/一般 3 天，见 `rules.ISSUE_DEADLINE_DAYS`）；显式传入的期限原样保留。前端表单的默认期限与后端推算同口径，用户手改后不再被覆盖。
- **整改闭环**：`待整改 → 整改中 → 待验收 → 已完成 → 已关闭`；`待验证` 阶段可被驳回退回 `整改中`，`待整改/整改中` 可直接作废关闭。每次流转都会写入一条整改流水（动作、原状态、新状态、操作人、说明），详情页以时间线呈现。
- **超期预警**：设有期限、期限早于当前时间且状态仍处于未闭环（待整改/整改中/待验收）三条同时满足才算超期。Python 判定（`rules.is_overdue`，用于标签/回显）与 SQL 条件（`rules.overdue_conditions`，用于列表筛选和看板计数）成对定义在同一处。
- **删除保护**：删除公厕时若已存在巡查或问题记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。

### 改造后必须成立的不变量

以下结论由 `backend/tests/test_rules.py` 固化，任何规则改动都必须继续满足：

1. **评分不变量**：同一组检查项，无论在录入接口、表单实时预览，还是任何读取入口，得分、等级、巡查结论完全相同；得分仅由检查项分数与 `rules.py` 常量决定。
2. **期限不变量**：同一严重程度与上报时间，前端默认值、录入落库值、演示数据（seed）推算出的期限相同；已显式填写或已落库的历史期限不被重算、不改写。
3. **超期不变量**：同一条问题，列表标签、列表 `overdue=true/false` 筛选、看板 `issue_overdue` 计数三处结论一致；且已完成/已关闭与无期限的问题永远不算超期。
4. **编号不变量**：所有编号都形如 `WT-YYYYMMDD-NNN`，同日唯一、三位流水号不重复。
5. **兼容性不变量**：页面操作、字段含义、接口返回结构（含 `/meta/dictionaries` 仅新增字段）与既有历史数据的结论保持不变。
